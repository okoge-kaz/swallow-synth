...
from src.static.style_rewriting_prompt import CPP_STYLE_REWRITING_PROMPT, CPP_SELF_CONTAINED_REWRITING_PROMPT


def syntax_check(code: str) -> bool:
    try:
        # Write code to temp file
        tmp_path = Path("tmp_code.cpp")
        tmp_path.write_text(code, encoding="utf-8")
        # Check syntax with g++
        result = subprocess.run(["g++", "-fsyntax-only", str(tmp_path)], capture_output=True, text=True)
...
def lint_with_cppcheck(code: str, tmp_path: Path) -> str:
    ...
    try:
        ...
        proc = subprocess.run(
            ["cppcheck", "--xml", "--xml-version=2", "--language=c++", str(tmp_path)],
            capture_output=True,
            text=True,
            check=False,
        )
...
def process_item_cpu(item: Dict[str, Any], key: str = "text") -> Dict[str, Any]:
    ...
    # 2) Fast format
    formatted: str = format_with_clang_format(code, Path("tmp_code.cpp"))
    item["text_formatted"] = formatted
    # 3) Lint
    lint_report: str = lint_with_cppcheck(formatted, Path("tmp_code.cpp"))
...
class RewritePipeline:
    ...
    def rewrite_style(self, code: str, lint_report: str) -> str:
        prompt = CPP_STYLE_REWRITING_PROMPT.format(lint_report=lint_report)
        return self.generate(prompt)

    def rewrite_self_contained(self, code: str) -> str:
        prompt = CPP_SELF_CONTAINED_REWRITING_PROMPT.format(code=code)
        return self.generate(prompt)
