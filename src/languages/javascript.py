...
def syntax_check(code: str) -> Tuple[bool, List[Dict[str, Any]]]:
    ...
    try:
        ...
        if proc.returncode == 0:
            return True, []
        message = (proc.stderr or proc.stdout or "node --check failed").strip()
...
def process_item_cpu(
    ...
    unique_path = tmp_dir / f"{uuid.uuid4()}.js"
    formatted, format_errors = format_with_prettier(code, parser="babel", tmp_path=unique_path)
