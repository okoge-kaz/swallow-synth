...
def syntax_check(code: str) -> Tuple[bool, List[Dict[str, Any]]]:
    with tempfile.NamedTemporaryFile(mode="w", suffix=".ts", delete=False, encoding="utf-8") as tmp_file:
        tmp_file.write(code)
        tmp_path = Path(tmp_file.name)

    try:
        proc = subprocess.run(
            ["tsc", "--noEmit", "--pretty", "false", str(tmp_path)],
            capture_output=True,
            text=True,
            check=False,
        )
        if proc.returncode == 0:
            return True, []
        message = (proc.stderr or proc.stdout or "tsc --noEmit failed").strip()
...
def format_with_prettier(
    ...
    try:
        proc = subprocess.run(
            ["prettier", "--stdin-filepath", str(path), "--parser", parser],
            input=code,
            capture_output=True,
            text=True,
            check=True,
        )
        formatted = proc.stdout or code
        return formatted, []
    except FileNotFoundError:
        return code, []
    except subprocess.CalledProcessError as exc:
        message = (exc.stderr or exc.stdout or "prettier failed").strip()
...
def process_item_cpu(
    ...
    unique_path = tmp_dir / f"{uuid.uuid4()}.ts"
    formatted, format_errors = format_with_prettier(code, parser="typescript", tmp_path=unique_path)
