...
def syntax_check(code: str) -> Tuple[bool, List[Dict[str, Any]]]:
    with tempfile.NamedTemporaryFile(mode="w", suffix=".cu", delete=False, encoding="utf-8") as tmp_file:
        tmp_file.write(code)
        tmp_path = Path(tmp_file.name)

    output_object = tmp_path.with_suffix(".o")
    try:
        proc = subprocess.run(
            ["nvcc", "-c", str(tmp_path), "-o", str(output_object)],
            capture_output=True,
            text=True,
            check=False,
        )
        if proc.returncode == 0:
            return True, []
        message = (proc.stderr or proc.stdout or "nvcc syntax check failed").strip()
...
def process_item_cpu(
    ...
    unique_path = tmp_dir / f"{uuid.uuid4()}.cu"
