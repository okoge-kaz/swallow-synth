...
## Word List:
- **Spread spectrum**: A method to spread signal energy over a wide frequency band to reduce interference.
- **Clock generator**: A device that produces periodic timing signals for synchronizing digital circuits.
- **EMI (Electromagnetic Interference)**: Unwanted disturbance generated by electromagnetic radiation from electronic devices.
- **Modulation**: The process of varying a signal's properties, here the frequency deviation.
- **Frequency excursion**: The amount by which the frequency deviates from the nominal value.
```
...
- **Quadratic formula**: The formula $x = \frac{-b \pm \sqrt{b^2 - 4ac}}{2a}$ used to find the roots of a quadratic equation.
- **Discriminant**: The expression $b^2 - 4ac$, which determines the nature of the roots (positive for two real roots, zero for one real root, negative for complex roots).
