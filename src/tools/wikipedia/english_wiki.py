...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--input-dir",
        type=str,
        help="Directory containing enwiki_namespace_0_*.ndjson files",
    )
    parser.add_argument(
        "--output-file",
        type=str,
...
def load_paths(input_dir: str) -> List[Path]:
    paths = []
    for path in Path(input_dir).glob("enwiki_namespace_0_*.ndjson"):
        [idx] = re.findall(r"enwiki_namespace_0_(\d+).ndjson", path.name)
...
def load_jsonl(example: Dict) -> Dict:
    ...
    is_disambiguation_page = any("Template:Dmbox" in template or "Template:Disambiguation" in template for template in templates)
...
SECTIONS_TO_IGNORE = ["Notes", "Further reading", "References", "See also", "External links"]
